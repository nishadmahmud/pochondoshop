...
export default function ProductCard({ product }) {
    // Generate a URL-friendly slug from the product name + id
    const nameSlug = product.name ? product.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') : 'product';
    const slug = product.id ? `${nameSlug}-${product.id}` : nameSlug;

    return (
        <Link href={`/product/${slug}`} className="bg-white border border-gray-100 rounded-[16px] pb-4 flex flex-col hover:border-brand-purple/30 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300 group overflow-hidden relative block">
            ...
            <div className="flex flex-col text-left px-3 pt-3">
                <h3 className="text-gray-800 font-bold text-[13px] md:text-[15px] leading-snug line-clamp-2 mb-1">
                    {product.name}
                </h3>

                <div className="flex items-center gap-2">
                    <span className="text-gray-800 font-extrabold text-[15px] md:text-[17px]">
                        {product.price}
                    </span>
                    {product.oldPrice && (
